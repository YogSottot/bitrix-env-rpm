...
# get_text variables
[[ -f $sphinx_menu_dir/functions.txt   ]] && . $sphinx_menu_dir/functions.txt
...
get_sphinx_servers_status() {
    ...
    for line in $POOL_SERVER_LIST; do
        ...
        if [[ $sphinx_version == "not_installed" ]];
        then
