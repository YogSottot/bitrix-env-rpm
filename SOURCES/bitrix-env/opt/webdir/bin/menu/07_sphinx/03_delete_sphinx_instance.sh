...
delete_sphinx_instance() {
    ...
    [[ $test_srv_name -eq 0 ]] && print_message "$(get_text "$SPH0207" "$srv_name")" "" "" any_key && return 1

    print_pool_sites "" "Y"
    print_message "$SPH0012" "" "" sphinx_dbname
    [[ $(echo "$POOL_SITES_KERNEL_LIST" | grep -c ":$sphinx_dbname:") -eq 0 ]] && print_message "$(get_text "$SPH0006" "$sphinx_dbname" )" "" "" any_key && return 2
...
sub_menu() {
    ...
    until [[ -n "$MENU_SELECT" ]]; do
        ...
        # print all
        print_sphinx_servers_status sphinx
