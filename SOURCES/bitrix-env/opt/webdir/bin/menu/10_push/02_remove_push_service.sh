...
push_instance(){
    ...
    if [[ $is_exist_host -eq 0 ]]; then
        print_message "$PUSH200" \
            "$(get_text "$PUSH206" "$hostname")" "" any_key
        return 1
    fi

    if [[ $hostname != "$NODE_PUSH_SERVER" ]]; then
        print_message "$PUSH200" \
            "$(get_text "$PUSH008" "$hostname")" \
            "" any_key
        return 1
    fi

    print_message \
        "$(get_text "$PUSH009" "$hostname")" \
        "" "" any_key "n"
...
sub_menu(){
    ...
    until [[ -n "$MENU_SELECT" ]]; do
        ...
        if [[ ( $POOL_SUBMENU_TASK_LOCK -gt 0 ) || \
             ( $print_push_servers_status_rtn -gt 0 ) || \
             ( -z $NODE_PUSH_SERVER ) ]]; then
