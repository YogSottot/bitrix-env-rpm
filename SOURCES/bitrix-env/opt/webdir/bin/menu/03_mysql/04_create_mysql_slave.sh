...
get_mysql_settings() {
    if [[ ! -f $ansible_mysql_group ]]; then
        print_message "$MY0200" "$(get_text "$MY0036" "$ansible_mysql_group")" "" any_key
        return 1
    fi
...
cluster_password=="
    ...
    for opt_portrait in $MYSQL_OPTIONS; do
        ...
        is_inventory_value_exists=$(grep -v '^$\|^#' $ansible_mysql_group | grep "^$my_key:" | awk -F':' '{print $2}' | grep -cv "^\s*$"  )
        [[ $DEBUG -gt 0 ]] && echo "$my_key => $is_inventory_value_exists"
        ...
        if [[ $(echo "$my_key" | grep -c "_password$") -gt 0 ]]; then
            ...
            print_message "$MY0037 $key_var1 $key_var2: " "" "" "user_choice" "$my_default"
            MYSQL_OPTIONS=$(echo "$MYSQL_OPTIONS" | \
    ...
    if [[ $DEBUG -gt 0 ]]; then
        for opt_portrait in $MYSQL_OPTIONS; do
            printf "%-10s: %s\n" "$(echo "$opt_portrait" | awk -F'=' '{print $1}')" "$(echo "$opt_portrait" | awk -F'=' '{print $2}')"
        done
    fi
}
