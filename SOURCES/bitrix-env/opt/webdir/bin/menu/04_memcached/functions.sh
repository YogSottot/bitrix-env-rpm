...
create_mc(){
    ...
    if [[ $test_server -eq 0 ]]; then
         print_message "$MC0200" \
             "$(get_text "$MC0206" "$mx_server")" "" any_key
         return 1
    fi
...
remove_mc(){
    print_mc_servers || return 1 
    print_message "$MC0003" "" "" mc_server
    test_server=$(echo "$POOL_SERVER_LIST" | \
        grep -w "memcached" | grep -cw "$mc_server")
    if [[ $test_server -eq 0 ]]; then
         print_message "$MC0200" \
             "$(get_text "$MC0206" "$mc_server")" "" any_key
         return 1
    fi
