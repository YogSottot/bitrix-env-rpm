...
configure_tr() {
    ...
    SITE_DIR=$(echo -e "$SITES_TR" | grep "^$SITE_NAME:" | awk -F':' '{print $2}')
    if [[ -z $SITE_DIR ]];
    then
        print_message "$TRANSF210" "$(get_text "$TRANSF014" "$SITE_NAME")" "" any_key
        return 1
    fi
    ...
    local task_exec="$bx_web_script -a configure_transformer --site $SITE_NAME --root $SITE_DIR --hostname $TR_CHOICE --domains $SITE_NAME,localhost"
