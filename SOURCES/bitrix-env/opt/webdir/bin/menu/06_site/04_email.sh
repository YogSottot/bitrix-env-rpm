...
update_email() {
    ...
    print_message "$SM0023 ($site_email_serv): " \
        "" "" site_email_serv $site_email_serv
    print_message "$SM0024 ($site_email_port): " \
        "" "" site_email_port $site_email_port
    ...
    print_message "$(get_text "$SM0025" "$site_email_serv" "$site_email_port")" \
        "" "" site_email_auth 'n'
    if [[ $(echo "$site_email_auth" | grep -wci 'y') -gt 0 ]]; then
        print_message "$SM0026 ($site_email_from): " \
            "" "" site_email_user $site_email_from
        ...
        print_message "$SM0028" "$SM0029 $supported_methods" \
            "" site_auth_method "auto"
        ...
        if [[ $(echo $supported_methods | grep -wc "$site_auth_method") -gt 0 ]]; then
            ...
            print_message "$CS0101" \
                "$(get_text "$SM0030" "$site_auth_method")" "" any_key
            return 1
        fi
    fi

    print_message "$(get_text "$SM0031" "$site_email_serv" "$site_email_port")" \
        "" "" site_email_tls_opt 'y'
    ...
    if [[ $? -gt 0 ]]; then
        ...
        print_message "$CS0101" \
            "$(get_text "$SM0032" "$site_name")" "" any_key
    fi
...
manage_email() {
    ...
    [[ -n "$site_email_status" ]] && \
        site_email_human_status="$(get_text "$SM0035" "$site_name")"

    print_color_text "$site_email_human_status" blue
    if [[ -n  "$site_email_status" ]]; then
        printf "%-20s: %s\n" "$SM0037" \
            $(echo "$site_info_dat" | awk -F':' '{print $5}')
        printf "%-20s: %s\n" "$SM0038" \
            $(echo "$site_info_dat" | awk -F':' '{print $6}')
        printf "%-20s: %s\n" "$SM0039" \
            $(echo "$site_info_dat" | awk -F':' '{print $7}')
        printf "%-20s: %s\n" "$SM0040" \
            $(echo "$site_info_dat" | awk -F':' '{print $8}')
        printf "%-20s: %s\n" "$SM0041" \
            $(echo "$site_info_dat" | awk -F':' '{print $10}')
        print_message "$(get_text "$SM0042" "$site_name")" \
            "" "" _update_email_answ 'n'
