...
delete_site() {
    ...
    if [[ ( $is_kernel_site -eq 0 ) && ( $is_link_site -eq 0 ) ]]; then
        ...
        if [[ $site_ok -eq 0 ]]; then
            ...
            if [[ $(echo "$POOL_SITES_LINK_LIST" | grep -c "Not found $site_dir/bitrix on the host") ]]; then
                # test site info, if we can found nginx and apache config we will delete it
                local site_name=$(basename $site_dir)
                local site_info=$($bx_sites_script -a status --site $site_name --root $site_dir | grep "^bxSite:configs:$site_name:" | sed -e "s/bxSite:configs:$site_name://")
                local ngx_cfg=$(echo "$site_info" | awk -F ':' '{print $1}')
                if [[ -z $ngx_cfg ]]; then
                    print_message "$CS0101" "$(get_text "$SM0003" "$site_name")" "" any_key
                    exit
                else
                    ...
                    printf "%-20s: %s\n" "$SM0007" "$apache_cfg"
                    printf "%-20s: %s\n" "$SM0008" "$site_dir"
                    [[ -d $php_sess_dir ]] && printf "%-20s: %s\n" "$SM0009" "$php_sess_dir"
                    [[ -d $php_upload_dir ]] && printf "%-20s: %s\n" "$SM0010" "$php_upload_dir"
                ...
                print_message "$CS0101" "$SM0014 $site_dir" "" any_key
                exit
            fi
        fi
    # site found in the list
    else
        [[ $is_kernel_site -gt 0 ]] && site_name=$(echo "$POOL_SITES_KERNEL_LIST" | grep ":$site_dir:" | awk -F':' '{print $1}')
        [[ $is_link_site -gt 0 ]] && site_name=$(echo "$POOL_SITES_LINK_LIST" | grep ":$site_dir:" | awk -F':' '{print $1}')
    ...
    if [[ $is_kernel_site -gt 0 ]]; then
        ...
        if [[ -n "$TR_INFO" && $TR_DIR == "$site_dir" ]]; then
            print_message "$TRANSF016" "$TRANSF012 $site_dir" "" any_key
            exit
        fi
    fi
...
menu_delete() {
    ...
    until [[ -n "$SITE_MENU_SELECT" ]]; do
        ...
        if [[ $POOL_SITE_TASK_LOCK -eq 1 ]]; then
            menu_list="$menu_delete_00"
        else
            menu_list="$menu_delete_01\n\t\t $menu_delete_00"
        fi
        print_menu
