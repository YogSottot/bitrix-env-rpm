...
manage_composite() {
    ...
    [[ $DEBUG -gt 0 ]] && \
        echo "site_name=$site_name composite=$site_composite nginx_settings=$site_nginx"

    if [[ "$site_composite" == "N" ]]; then
        print_message "$CS0100" \
            "$(get_text "$SM0110" "$site_name")" \
            "" any_key
    else
        ...
        if [[ "$site_nginx" == "Y" ]]; then
            # 1
            print_message "$(get_text "$SM0111" "$site_name")" \
                "" "" site_answer 'y'
            ...
            # 2
                ...
                print_message "$(get_text "$SM0112" "$site_name")" \
                    "" "" site_answer 'n'
                if [[ $(echo "$site_answer" | grep -icw 'y') -gt 0 ]]; then
        ...
        if [[ -n "$service_site_exe" ]]; then
            exec_pool_task "$service_site_exe" \
                "$(get_text "$SM0113" "$service_site_status")"
        else
